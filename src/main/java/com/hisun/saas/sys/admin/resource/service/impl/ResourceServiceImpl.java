...
public class ResourceServiceImpl extends
	...
	public void updateResources(Resource resource, String oldPid,
		...
		boolean oldSysResource = resource2.getSysResource();
		boolean newSysResource = resource.getSysResource();
		...
		if(oldSysResource!=newSysResource){
			extracted(resource, newSysResource,type);
		}
	}

	private void extracted(Resource resource, boolean newSysResource,int type) {
		...
		for(Resource resource3 : resources){
			resource3.setSysResource(newSysResource);
			this.resourceDao.update(resource3);
			extracted(resource3, newSysResource,type);
		}
	}
