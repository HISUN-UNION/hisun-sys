...
<body>
	...
	<script type="text/javascript">
		...
		var setting1 = {
				async: {
					enable: true,
					url:"${path}/sys/tenant/resource/select/tree",
					dataType : "json",
					headers: {
						"OWASP_CSRFTOKEN":"${sessionScope.OWASP_CSRFTOKEN}"
					},
		...
		function dirSel(treeId,treeNode){
			...
			$('#pName').val(text);
			$.get("${path}/sys/tenant/resource/max/sort/",{"pId":treeNode.id},function(data){
		...
		function onClick (event, treeId, treeNode){
			$("#pId").val(treeNode.id);
			$.ajax({
				url : "${path}/sys/tenant/resource/sitemesh/list?pId="+treeNode.id,
				type : "get",
				data : null,
				dataType : "html",
				headers: {
		...
		function treeLoad(){
			$.ajax({
				...
				url: "${path}/sys/tenant/resource/select/tree",// 请求的action路径
		...
		$(document).ready(function(){
			...
			$("#submit").on("click",function(event){
				...
				localPost("${path}/sys/tenant/resource/permission/check",{
					...
					if(!data.success){
						...
						if(bool){
							if($("#id").val()===""){
								$.ajax({
									url : "${path}/sys/tenant/resource/add",
									type : "post",
									...
									success : function(json){
										if(json.success){
											...
											$("#add").hide();
											refreshTree();
											//$("#listResource").load("${path}/sys/tenant/resource/sitemesh/list?pId="+$("#pId").val());
											$.ajax({
												cache:false,
												type: 'POST',
												dataType : "html",
												...
												url: "${path}/sys/tenant/resource/sitemesh/list?pId="+$("#pId").val(),// 请求的action路径
												error: function () {// 请求失败处理函数
													alert('请求失败');
												},
												success:function(html){
								...
								$.ajax({
									url : "${path}/sys/tenant/resource/update",
									type : "post",
									data : $("#addForm").serialize(),
									dataType : "json",
									headers: {
			...
			$.ajax({
				...
				type: 'POST',
				dataType : "json",
				url: "${path}/sys/tenant/resource/tree",// 请求的action路径
				headers: {
					"OWASP_CSRFTOKEN":"${sessionScope.OWASP_CSRFTOKEN}"
				},
				error: function () {// 请求失败处理函数
				...
				success:function(data){
					if(data.success){
						...
						//$("#listResource").load("${path}/sys/tenant/resource/sitemesh/list?pId=1");
						$.ajax({
							cache:false,
							type: 'POST',
							dataType : "html",
							...
							url: "${path}/sys/tenant/resource/sitemesh/list?pId=1",// 请求的action路径
							error: function () {// 请求失败处理函数
								alert('请求失败');
							},
							success:function(html){
		...
		function pagehref (pageNum ,pageSize){
			/*$.get("<%=path%>/sys/tenant/resource/sitemesh/list?pId="+$('#pId').val()+"&pageNum="+pageNum+"&pageSize="+pageSize,function(data){
			...
			$.ajax({
				...
				url: "<%=path%>/sys/tenant/resource/sitemesh/list?pId="+$('#pId').val()+"&pageNum="+pageNum+"&pageSize="+pageSize,// 请求的action路径
				error: function () {// 请求失败处理函数
					alert('请求失败');
				},
				success:function(html){
