				...
				<div class="portlet-title">
					...
					<div class="clearfix fr">
							<span class="controllerClass btn green file_but" >
								<i class="icon-circle-arrow-up"></i>上传职数
							...
							<%--</span>--%>
								<%--<a id="sample_editable_1_new" class="btn green" href="${path }/zzb/app/console/appZscxZs/add">--%>
					...
					<table class="table table-striped table-bordered table-hover dataTable table-set">
						...
						<c:forEach items="${pager.datas}" var="vo">
							<tr style="text-overflow:ellipsis;">
								<%--<td title="${vo.xm}"><a href="${path}/zzb/app/console/appZscxZs/viewA01s?id=${vo.id }"><c:out value="${vo.xm}"></c:out></a></td>--%>
									<c:if test="${empty b01Id}">
										<td title="${vo.b0101}"><c:out value="${vo.b0101}"></c:out></td>
									</c:if>

								<td title="${vo.zwmc}"><a href="javascript:viewA01s('${vo.id }')" class=""><c:out value="${vo.zwmc}"></c:out></a></td>
								<td title="${vo.xp}"><c:out value="${vo.xp}"></c:out></td>
								<td title="${vo.sp}"><c:out value="${vo.sp}"></c:out></td>
								<td title="${vo.cqb}"><c:out value="${vo.cqb}"></c:out></td>
								<%--<td class="Left_alignment">--%>
									<%--<a href="javascript:del('${vo.id }','${vo.zwmc}')" class="">删除</a>--%>
								<%--</td>--%>
							</tr>
						</c:forEach>
						</tbody>
